...
class CustomerManager(models.Manager):
    ...
    def import_state(self, state_name, business):
        state = N_State.objects.filter(name=state_name).first()
        if not  state:
            state = business.address.state
        return state

    def extract_data_from_record(self, record, business):
        birthday = record.get('Fecha de Nacimiento')
        state_value = record.get('Estado')
        state = self.import_state(state_name=state_value, business=business)
        # if birthday:
        ...
        return {
            ...
            'first_line':record.get('Line 1'),
            'second_line': record.get('Line 2'),
            'city': record.get('Ciudad'),
            'zip': record.get('Codigo Postal'),
            'state': state
        }
    ...
    def export_to_excel(self, business):
        ...
        return [{'Nombre':cust.first_name, 'Apellidos':cust.last_name,
          ...
          'Telefono':cust.cellphone,
          'Telefono Casa':cust.phone_home,
          'Line 1': cust.address.first_line,
          'Line 2': cust.address.second_line,
          'Ciudad': cust.address.city,
          'Pais': cust.address.state.country.name,
          'Estado': cust.address.state.name,
          'Codigo Postal': cust.address.zip
                 }
                for cust in query]
        ...
        try:
            ...
            for rec in records:
                if not escape_headers:
                    escape_headers = True
                    continue
                data = self.extract_data_from_record(record=rec, business=business)
...
class BusinessManager(models.Manager):
    ...
    def update_business_data(self, business, data, logo=None):
        ...
        try:
            ...
            business.address.update(data=data)
            business.save()
            return business
        except ValidationError:
            return self.EMAIL_ERROR
...
class Business(models.Model, ModelSerialize):

    default_fields = ['name','tax','phone','trial_end','websiteurl','default_site_mine','capacity','address',
                      'owner','logo','stripecustomer','stripe_account','can_choose_default_site','has_been_subscribed']
    ...
    def serializable_value(self, field_name):
        ...
        if field_name == 'stripecustomer':
            if self.stripecustomer:
                return self.stripecustomer.serialize()
            return None
        if field_name == 'stripe_account':
            if self.stripe_account:
                return self.stripe_account.serialize()
            return None
        if field_name =='tax':
            return float(self.tax)
        if field_name == 'can_choose_default_site':
            return self.can_choose_default_site()
        if field_name == 'has_been_subscribed':
            return self.has_been_subscribed()

        return super(Business, self).serializable_value(field_name)

    def can_accepted_one_more(self, current_number_of_events):
    ...
    def can_choose_default_site(self):
        return self.websiteurl and self.get_website() and True
    ...
    def get_trial_end_value_to_subscribe(self):
        if self.current_group().name == settings.BUSINESS_BASIC:
            ...
            return (int(datetime(year=self.trial_end.year, month=self.trial_end.month, day=self.trial_end.day).timestamp()))
    ...
    def change_group(self, old_group_name, new_group_name):
        remain = self.owner.groups.exclude(name=old_group_name)
...
class Customer(models.Model, ModelSerialize):
    ...
    default_fields = ['prefix','suffix','first_name','last_name','email','company','comments','business','activo','birthday',
                      'cellphone','phone_home','address','new','id','full_name']
...
class Address(models.Model, ModelSerialize):
    ...
    default_fields = ['first_line','second_line','city','zip','country', 'state','id','checked']
