...
class StripeCustomer(models.Model, ModelSerialize):
    ...
    default_fields = ['stripeid','stripetoken','billing_email', 'subscription','card']

    def serializable_value(self, field_name):
        if field_name == 'card':
            return self.get_card().serialize()
        if field_name == 'subscription':
            return self.current_subscription().serialize()
        return super(StripeCustomer, self).serializable_value(field_name)
...
class StripeCard(models.Model, ModelSerialize):
    ...
    default_fields = ['stripe_exp_month','stripe_exp_year','stripe_last4','billing_address']

    def serializable_value(self, field_name):
        if field_name == 'billing_address':
            return self.get_billing_address()
        return super(StripeCard, self).serializable_value(field_name)
    ...
    def update_from_stripe(self, stripe_card):
        print(stripe_card)
        print(stripe_card.__class__)
        print(stripe_card.id)
        print(stripe_card.exp_month)
        print(stripe_card.exp_year)
        print(stripe_card.last4)
        print(stripe_card.brand)
        print(stripe_card.address_country)
        print(stripe_card.address_city)
        print(stripe_card.address_state)
        print(stripe_card.address_line1)
...
class StripeSubscripcion(models.Model, ModelSerialize):
    TRIAL = 'trialing'
    ACTIVE = 'active'
    PAST_DUE = 'past_due'
    CANCELED = 'canceled'
    ...
    default_fields = ['current_period_start','current_period_end', 'billing_receipts']

    def serializable_value(self, field_name):
        if field_name == 'billing_receipts':
            return serialize_query(self.get_billing_receipts())
        return super(StripeSubscripcion, self).serializable_value(field_name)
...
class StripeInvoice(models.Model, ModelSerialize):
    ...
    default_fields = ['created_at','receipt_number']
...
class StripeAccount(models.Model, ModelSerialize):
    ...
    default_fields = ['stripe_publishable_key','access_token','stripe_user_id']
